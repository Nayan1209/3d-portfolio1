...
function Cube({ position, scale = [1, 1, 1], color }: { position: [number, number, number]; scale?: [number, number, number]; color: string }) {
  return <mesh position={position} scale={scale} castShadow receiveShadow><boxGeometry args={[1, 1, 1]} /><meshStandardMaterial color={color} roughness={0.85} /></mesh>;
}
...
function Sign() {
  return (
    <group position={[0, 2.1, -3]}>
      ...
      <Float speed={1.2} floatIntensity={0.25}><Text position={[0, 2, 0]} fontSize={0.25} color="#dce7df" anchorX="center">Explore. Build. Capture.</Text></Float>
    </group>
  );
}
...
function Particles({ color }: { color: string }) {
  ...
  useFrame(({ clock }) => { if (ref.current) ref.current.rotation.y = clock.elapsedTime * 0.8; });
  return <group ref={ref}>{Array.from({ length: 8 }, (_, i) => <mesh key={i} position={[Math.cos(i) * 2, 1.4 + (i % 3) * 0.35, Math.sin(i) * 2]}><boxGeometry args={[0.12, 0.12, 0.12]} /><meshBasicMaterial color={color} /></mesh>)}</group>;
}
  ...
  return (
    <group position={[zone.x, 0, zone.z]} {...common}>
      {zone.id === 'skills' ? <>
        ...
        <Cube position={[0, 1.4, 0]} scale={[5, 2.8, 4]} color="#262426" /><Cube position={[0, 3, 0]} scale={[5.4, 0.35, 4.4]} color="#171617" />
        ...
        <Cube position={[-2, 1.6, 0]} scale={[0.7, 3.5, 0.7]} color="#552b54" /><Cube position={[2, 1.6, 0]} scale={[0.7, 3.5, 0.7]} color="#552b54" /><Cube position={[0, 3.05, 0]} scale={[4.7, 0.7, 0.7]} color="#552b54" />
        <mesh position={[0, 1.6, 0]}><boxGeometry args={[3.4, 3, 0.35]} /><meshStandardMaterial color="#b94383" emissive="#7b1f5c" emissiveIntensity={0.7} transparent opacity={0.72} /></mesh>
        <Text position={[0, 4.05, 0]} fontSize={0.55} color="#ffd2eb" anchorX="center">CONTACT</Text>
      </> : <>
        <Cube position={[0, 0.8, 0]} scale={[4, 1.6, 3.4]} color="#805538" /><Cube position={[0, 2, 0]} scale={[4.4, 0.55, 3.8]} color="#9d3e3e" />
        <Cube position={[0, 0.7, -1.76]} scale={[1.1, 1.4, 0.25]} color="#282322" /><Cube position={[-1.25, 1.15, -1.82]} scale={[0.55, 0.55, 0.2]} color="#6ab4d1" /><Cube position={[1.25, 1.15, -1.82]} scale={[0.55, 0.55, 0.2]} color="#6ab4d1" />
...
function World({ onInteract, onHover, onNear, touchRef }: { onInteract: (id: string) => void; onHover: (id: string | null) => void; onNear: (zone: Zone | null) => void; touchRef: MutableRefObject<TouchState> }) {
  ...
  const yaw = useRef(0), pitch = useRef(-0.06), velocityY = useRef(0), grounded = useRef(true);
  useEffect(() => {
    ...
    const move = (event: MouseEvent) => { if (document.pointerLockElement === gl.domElement) { yaw.current -= event.movementX * 0.0022; pitch.current = THREE.MathUtils.clamp(pitch.current - event.movementY * 0.0018, -1.2, 1.2); } };
  ...
  useFrame((_, delta) => {
    ...
    velocityY.current -= 16 * delta; camera.position.y += velocityY.current * delta;
    if (camera.position.y <= 1.15) { camera.position.y = 1.15; velocityY.current = 0; grounded.current = true; }
    yaw.current -= touch.lookX * 0.006; pitch.current = THREE.MathUtils.clamp(pitch.current - touch.lookY * 0.004, -1.2, 1.2); touch.lookX = 0; touch.lookY = 0;
    camera.rotation.set(pitch.current, yaw.current, 0, 'YXZ');
    const result = nearest(camera.position); onNear(result && result.distance < 4 ? result.zone : null);
  });
  return <><ambientLight intensity={0.8} /><directionalLight position={[8, 16, 6]} intensity={2.5} castShadow shadow-mapSize-width={1024} shadow-mapSize-height={1024} /><hemisphereLight intensity={0.35} color="#d8f0ff" groundColor="#31462b" /><Terrain /><Sign />{zones.filter((zone) => zone.id !== 'spawn').map((zone) => <Building key={zone.id} zone={zone} onHover={onHover} onInteract={onInteract} />)}</>;
}
...
function TouchControls({ touchRef }: { touchRef: MutableRefObject<TouchState> }) {
  ...
  return <div className="touch-ui"><div className="joystick" ref={joystick} onPointerMove={updateJoystick} onPointerDown={(event) => { event.currentTarget.setPointerCapture(event.pointerId); updateJoystick(event); }} onPointerUp={() => { touchRef.current.jx = 0; touchRef.current.jy = 0; }}><i /></div><div className="look-pad" onPointerMove={(event) => { if (event.buttons) { touchRef.current.lookX += event.movementX; touchRef.current.lookY += event.movementY; } }}><span>LOOK</span></div></div>;
}
...
export default function App() {
  ...
  useEffect(() => { const timer = window.setTimeout(() => setLoading(false), 900); return () => window.clearTimeout(timer); }, []);
  useEffect(() => { const timer = window.setInterval(() => setDay((value) => (value + 0.012) % 1), 1000); return () => window.clearInterval(timer); }, []);
  useEffect(() => { const handler = (event: KeyboardEvent) => { if (event.key.toLowerCase() === 'e' && near) setPanel(near.id); if (event.key === 'Escape') setPanel(null); }; window.addEventListener('keydown', handler); return () => window.removeEventListener('keydown', handler); }, [near]);
  ...
  return <main className="game-shell">
    ...
    <div className="minimap"><span className="map-title">WORLD MAP</span><span className="map-player" />{zones.filter((zone) => zone.id !== 'spawn').map((zone) => <button key={zone.id} className="map-dot" style={{ left: `${50 + zone.x * 2.1}%`, top: `${50 + zone.z * 2.1}%`, color: zone.color, background: zone.color }} onClick={() => setPanel(zone.id)} aria-label={zone.label} />)}</div>
    {near && <div className="prompt"><kbd>E</kbd><strong>{near.label}</strong><span>INTERACT</span></div>}
    {hover && <div className="hover-label">{zones.find((zone) => zone.id === hover)?.label}</div>}
