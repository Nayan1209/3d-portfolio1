...
function Cube({ position, scale = [1, 1, 1], color, onClick }: { position: [number, number, number]; scale?: [number, number, number]; color: string; onClick?: () => void }) {
  return (
    <mesh position={position} scale={scale} castShadow receiveShadow onClick={onClick}>
...
function Terrain() {
  ...
  return (
    <group>
      {blocks.map(([x, y, z, color], i) => <Cube key={i} position={[x, y, z]} color={color} />)}
    </group>
  );
}
...
function ZoneBuilding({ zone, onInteract }: { zone: Zone; onInteract: (id: string) => void }) {
  ...
  if (zone.id === 'skills') {
    return (
      <group position={[zone.x, 0, zone.z]}>
        {[0, 1, 2, 3, 4, 5].map((i) => (
          <Cube key={i} position={[0, i + 0.5, 0]} scale={[3.2 - i * 0.18, 1, 3.2 - i * 0.18]} color={i % 2 ? '#7955a9' : '#b795e8'} onClick={click} />
        ))}
        <Cube position={[0, 6.7, 0]} scale={[2.1, 0.35, 2.1]} color={glow} onClick={click} />
      </group>
    );
  }
  ...
  return (
    <group position={[zone.x, 0, zone.z]} onPointerOver={() => setHovered(true)} onPointerOut={() => setHovered(false)}>
...
function World({ onInteract, onNear, touchRef }: { onInteract: (id: string) => void; onNear: (zone: Zone | null) => void; touchRef: React.MutableRefObject<TouchState> }) {
  ...
  useEffect(() => {
    ...
    const move = (event: MouseEvent) => {
      ...
      pitch.current = THREE.MathUtils.clamp(pitch.current - event.movementY * 0.0018, -1.15, 1.15);
    };
    const lock = () => {
  ...
  return (
    <>
      ...
      <hemisphereLight intensity={0.45} color="#d8f0ff" groundColor="#31462b" />
      <directionalLight position={[8, 18, 7]} intensity={2.8} castShadow shadow-mapSize-width={1024} shadow-mapSize-height={1024} />
      <Terrain />
      <SpawnSign />
...
function TouchControls({ touchRef }: { touchRef: React.MutableRefObject<TouchState> }) {
  const joystickStart = useRef({ x: 0, y: 0 });
  return <div className="touch-ui">
    <div className="joystick" onPointerDown={(e) => { e.currentTarget.setPointerCapture(e.pointerId); joystickStart.current = { x: e.clientX, y: e.clientY }; }} onPointerMove={(e) => { if (!e.currentTarget.hasPointerCapture(e.pointerId)) return; const dx = e.clientX - joystickStart.current.x; const dy = e.clientY - joystickStart.current.y; const d = Math.min(38, Math.hypot(dx, dy)); const a = Math.atan2(dy, dx); touchRef.current.jx = Math.cos(a) * d / 38; touchRef.current.jy = Math.sin(a) * d / 38; }} onPointerUp={() => { touchRef.current.jx = 0; touchRef.current.jy = 0; }} onPointerCancel={() => { touchRef.current.jx = 0; touchRef.current.jy = 0; }}><i /></div>
